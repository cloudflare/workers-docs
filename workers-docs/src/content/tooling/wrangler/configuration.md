...
**Top level only**: required to only be configured at the top level configure

**Top level OR Environment**: May be configured at the top level and/or the environment level. If both are specified then the field is determined by the environment level field.
...
| [`vars`](#vars)                   | Environment Only TODO: verify | An object containing text variables that can be directly accessed in a Worker script. | Optional      |
