...
test('test new redirect maps to updated URL', async t => {
  mockGlobal()
  const event = getEvent(new Request(`${origin}/reference/workers-concepts/security/`))
...
test('test old redirect maps to new URL', async t => {
  mockGlobal()
  const event = getEvent(new Request(`${origin}/workers/recipes/`))
  const res = await handleRequest(event)
  t.is(res.status, 301)
...
test('test old redirect for archives always maps to new URL', async t => {
  mockGlobal()
  const event = getEvent(new Request(`${origin}/workers/archive/recipes/static-site/`))
  const res = await handleRequest(event)
  t.is(res.status, 301)
...
test('canonical directory redirect', async t => {
  mockGlobal()
  const event = getEvent(new Request(`${origin}/workers/templates`))
  const res = await handleRequest(event)
  t.is(res.status, 301)
  t.is(res.headers.get('location'), `${origin}/workers/templates/`)
})
...
test('if URL does not exists, then redirect to root', async t => {
  mockGlobal()
  const event = getEvent(new Request(`${origin}/workers/blah/`))
  const res = await handleRequest(event)
  t.is(res.status, 301)
...
test('if archive does  exist, then show that archived doc', async t => {
  mockGlobal()
  const event = getEvent(new Request(`${origin}/workers/archive/recipes/a-b-testing/`))
  const res = await handleRequest(event)
  t.is(res.status, 301)
...
test('if archive does not exist, and it was redirected have new link', async t => {
  mockGlobal()
  const event = getEvent(new Request(`${origin}/workers/archive/recipes/pre-shared-keys/`))
  const res = await handleRequest(event)
  t.is(res.status, 200)
  t.true(!!(await res.text()))
})
