...
In addition to the properties on the standard [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request#Properties) object, you can use a the  `request.cf` object to control how Cloudflare features are applied as well as other custom information provided by Cloudflare. 
...
Setting cf is kind of like setting headers when [modifying a request](/reference/workers-concepts/modifying-requests/). You can add cf to a request object by making a copy. 
```javascript
